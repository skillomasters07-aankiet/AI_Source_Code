...
with header_col1:
    st.markdown("<div class='big-title'>📋 Meeting Minutes — Presenter Mode</div>", unsafe_allow_html=True)
    st.markdown("<div class='muted'>Clean, presentable UI for demos and hackathons — editable minutes, history, dashboards and quick AI analysis.</div>", unsafe_allow_html=True)
with header_col2:
...
with st.sidebar:
    ...
    st.markdown("---")
    st.header("Debug / Recovery")
    st.write("If parsing failed, try asking the model to clean its previous output:")
    if st.button("Retry parse LLM output (try cleaner)"):
        raw_prev = st.session_state.get("raw_output", "")
        parsed_fix, cleaned_raw = ask_llm_to_clean(raw_prev, temperature=0.0)
        if parsed_fix:
            minutes = default_minutes()
            if isinstance(parsed_fix, dict):
                minutes.update(parsed_fix)
            for k in default_minutes().keys():
                if k not in minutes:
                    minutes[k] = default_minutes()[k]
            st.session_state.minutes = minutes
            st.session_state.raw_output = cleaned_raw or raw_prev
            st.success("Successfully extracted JSON from LLM output.")
        else:
            st.error("Retry failed — LLM could not produce valid JSON from its previous response.")
...
# --- ONE-PAGE SUMMARY ---
with tabs[1]:
    st.subheader("One-page Summary (Presenter View)")
...
with tabs[4]:
    ...
    if st.session_state.analysis is None:
        ...
        st.write(f"**Sentiment:** {a.get('sentiment','unknown')}")
        st.write("**Risk Flags:**")
...
st.sidebar.markdown("---")
if st.sidebar.button("Download current minutes JSON"):
    st.sidebar.download_button("Download JSON", data=json.dumps(st.session_state.minutes, indent=2), file_name="meeting_minutes.json", mime="application/json")

st.sidebar.markdown("\n---\nPresenter mode: polished layout for quick demos. Want a PPT export, custom theme, or DB persistence? Reply with which you'd like and I will add it.")
